---
title: Quickstart
description: Make your first Trace OCR request
---
...
const response = await fetch("https://api.trace.so/ocr/", {
  method: "POST",
  body: form,
});
...
```bash
curl -X POST "https://api.trace.so/ocr/?detection_model=fast_base&recognition_model=parseq&binary_threshold=0.3" \
  -F "files=@document.jpg"
```
